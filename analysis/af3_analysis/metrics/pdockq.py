...
"""
pDockQ score calculation from https://gitlab.com/ElofssonLab/FoldDock
"""
...
# parser.add_argument('--output', nargs=1, type= str, default=sys.stdin, help = 'Path to output file to store pDockQ results')
...
if __name__ == '__main__':
    ...
    with open(input_list) as f:
        pdb_flist = f.read().splitlines()

    with open(output_fpath, 'a') as f:
        f.write('\t'.join(['ppi', 'pDockQ', 'PPV', 'model', 'filename']) + '\n')

    for pdbfile in tqdm(pdb_flist):
        ppi = os.path.dirname(pdbfile).split('/')[-1]
        ...
        with open(output_fpath, 'a') as f:
            f.write(f'{ppi}\t{pdockq}\t{ppv}\t{model_name}\t{fname}\n')
